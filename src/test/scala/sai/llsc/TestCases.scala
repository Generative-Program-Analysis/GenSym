...
object TestCases {
  ...
  val filesys: List[TestPrg] = List(
    ...
    TestPrg(statSymTest, "statSymBr1", "@main", noArg, "--add-sym-file A", nPath(1)++status(0)),
    TestPrg(statSymTest, "statSymBr2", "@main", noArg, "--add-sym-file A --add-sym-file B", nPath(2)++status(0)),
