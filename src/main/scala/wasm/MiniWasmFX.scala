...
case class EvaluatorFX(module: ModuleInstance) {
  ...
  trait Cont[A] {
    def apply(stack: Stack, mcont: MCont[A]): A
  }
  ...
  def initK[Ans](s: Stack, mkont: MCont[Ans]): Ans = mkont(s)
  ...
  def eval1[Ans](inst: Instr, stack: Stack, frame: Frame,
    ...
    inst match {
      ...
      case TryCatch(es1, es2) =>
        ...
        evalList(es1, List(), frame, initK[Ans], m1, List(), List((-1, newHandler)) ++ hs)
      case Resume0() =>
        val (resume: ContV[Ans]) :: newStack = stack
        resume.k(List(), kont, mkont, List())
      case Throw() =>
        val err :: newStack = stack
        def kr(s: Stack, k1: Cont[Ans], m1: MCont[Ans], hs: Handlers[Ans]): Ans = kont(s, s1 => k1(s1,  m1))
        hs.head._2(List(err, ContV(kr)), initK[Ans], mkont)

      // WasmFX effect handlers:
      case ContNew(ty) =>
        val RefFuncV(f) :: newStack = stack
        def kr(s: Stack, k1: Cont[Ans], m1: MCont[Ans], hs: Handlers[Ans]): Ans = {
          evalCall1(f, s, frame/*?*/, k1, m1, List(), hs, false)
        }
        kont(ContV(kr) :: newStack, mkont)
      case Suspend(tagId) =>
        val FuncType(_, inps, out) = module.tags(tagId)
        val (inputs, restStack) = stack.splitAt(inps.size)
        val kr = (s: Stack, k1: Cont[Ans], m1: MCont[Ans], hs: Handlers[Ans]) => {
          // FIXME: handlers are lost here
          kont(s ++ restStack, s1 => k1(s1, m1)) // mkont lost here
        }
        val newStack = ContV(kr) :: inputs
        hs.find(_._1 == tagId) match {
          case Some((_, handler)) => handler(newStack, initK[Ans], mkont)
      ...
      case ContBind(oldContTyId, newConTyId) =>
        ...
        def kr(s: Stack, k1: Cont[Ans], mk: MCont[Ans], handlers: Handlers[Ans]): Ans = {
          f.k(s ++ inputs, k1, mk, handlers)
        }
        kont(ContV(kr) :: restStack, mkont)
  ...
  def evalTop[Ans](halt: Cont[Ans], mhalt: MCont[Ans], main: Option[String] = None): Ans = {
    ...
    evalList(instrs, List(), frame, halt, mhalt, List(halt), List())
  }
