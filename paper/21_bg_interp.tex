...
A trait poses monadic operations as abstract methods that objects extending it
should implement. An object encapsulates the data and implements monadic methods
that operate on these data. The monadic @bind@ operation is usually called
@flatMap@ in Scala. For example, the following trait @Monad@ declares the
@return@ and @flatMap@ operation:
...
An object may encapsulate the data of type @F[A]@ and only provide
@flatMap(f: A => F[B]): F[B]@. This allows us to utilize the for-comprehensions in
Scala, which provide special syntactic support for moandic @flatMap@.
For example, a list object of element type @A@ is a monad and it implements the
methods @flatMap[B](f: A => List[B]): List[B]@ and @map[B](f: A => B): List[B]@.
To compute the Cartesian product of two lists of numbers, we may directly call
@flatMap@ and pass an anonymous function that internally calls the monadic
@return@ operation (the @List@ constructor in this case):
...
In the rest of the paper, we will use for-comprehension syntax for monadic
operations, the implementation of monads and monad transformers essentially
borrows the ground-truth from Haskell.
