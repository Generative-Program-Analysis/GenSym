...
Since we are mostly interested in analyzing dynamic behaviors of the
program, we disguise the static semantics. We also assume that
...
itself takes the monad type @M[_]@ as an argument, which is a higher-kinded type
...
in @for@-comprehensions is translated into a @map@, where the expression of
...
@MonadOps@ can be done by using the implicit design pattern.
...
@R@ with an identity type (i.e., @type R[T] = T@), then the generic interpreter is a
...
interpreter acts as a compiler, we will replace the monads to the ones that
...
value for performing effects. For examples, @local_env@ installs a new environment
...
the monad transformer approach \cite{DBLP:conf/popl/LiangHJ95}, a @ReaderT@ and
@StateT@ monad transformer are used to compose the monad stack. In other words,
...
part of the control flow. Although the way we treat them is simple now, it
...
When applying a function, @ap_clo@ takes a function value @rator@ and an argument
