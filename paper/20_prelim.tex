...
@List[A]@ is a @List@ monad, where @A@ is the element type.
...
@yield@ becomes the body expression inside of that @map@ operation. The foregoing
...
Therefore it only exposes the simplified version of @flatMap@, where the monad
@M[A]@ is not introduced as a function argument. The trait @MonadOps@ (Figure
...
@MonadOps@ can be done by using implicit design pattern.
...
@Store@. @Env@ maps from identifiers to addresses, and @Store@ maps from
...
still abstract, i.e., they are declared as abstract types.
\begin{lstlisting}
  trait Semantics {
...
The binding-time is declared as a higher-kinded type @R[_]@. 
Now we are able to use @R@ to annotate other data-types in the interpreter;
it is also injected into the monadic type @MonadOps@.  If we simply instantiate
@R@ an identity type (i.e., @type R[T] = T@), then the generic interpreter is a
standard definitional interpreter that will execute the program.  In Section
\ref{stagedinterp}, we will instantiate @R@ using LMS's built-in next-stage
...
@Ans@, which is a monadic type @AnsM[_]@ wrapping the type @Value@. 
...
type parameter @R[_]@ as binding-time, i.e., @MonadOps[R, AnsM, T]@.
Inside of @MonadOps@, @R[_]@ wraps the data types @A@ and @B@ that are
encapsulated by the monad, but not the monad type @M@ itself. When acting as
compilers, we will also replace the monads to the ones that work on staged data
values.
...
may parameterized over the environment or store type, or simply a @Unit@
...
of the addresses, therefore can be used in monadic computations.
...
Other primitive operations in the interpreter handles language constructs.  The
method @num@ and @close@ deal with primitive values, which lift literal terms
...
\paragraph{The Interpreter} Now we can construct the semantics-agnostic interpreter
...
Finally, a top-level wrapper @run@ is declared; the return type @Result@
depends on what kind of monads is being used, therefore this is also an
abstract type.
\begin{lstlisting}
...
interpreter with environments and stores. It can also be obtained by the
...
than the size of current store.
...
\cite{DBLP:conf/popl/LiangHJ95}, a @ReaderT@ and @StateT@ monad
transformer are used to compose the monad stack. In other words, the type @AnsM@ is
...
@Env@, and @StateT@ is parameterized by type @Store@, and the inner-most monad @IdM@
...
functions and applications. This is because lambda terms are data but are also
part of the control flow. Although the way we treat them is simple now, but it
will be very different when staging and abstract interpretation is involved.
...
@rand@. It extracts the lambda term and environment enclosed in @rator@, and
