...
stores a staged value of type @Rep[Set[A]]@, inside of a staged monad @M@.
\todo{staged SetT implementation}
...
\begin{lstlisting}
  case class IRCompiledClo(f: (Rep[Value], Rep[Store], Rep[Cache], Rep[Cache])
                           => Rep[(Set[(Value, Store)], Cache)]) extends Rep[AbsValue]
  case class IRApClo(clo: Rep[AbsValue], arg: Rep[Value], σ: Rep[Store],
                     in: Rep[Cache], out: Rep[Cache]) extends Rep[(Set[(Value, Store)], Cache)]
\end{lstlisting}
