...
it is not that easy when introducing the nondeterminism monad (@ListT@) for
...
the lambda term instead of generating a @CloV@ value. To make this happen, when
closing the lambda term with the environment, we need to staging into the body
expression by \textit{collpasing} the @AnsM@ monads to normal values.
The following function @close@ illustrates the idea:
