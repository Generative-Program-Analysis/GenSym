...
The function @f@ passed to @flatMap@ is also a function value known at the current stage.
...
@emitValDef@ for them.  For compiled closures, we put the function @f@ and its
accompanying syntactic term and environment to their next-stage value
representation @CompiledClo@. For applications, we know the fact that @fun@ is
