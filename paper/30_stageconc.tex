...
In @close@, we first create a function @f@, which input and output are both
next-stage values and stores.  The function @f@ is a current-stage and we then
use @emit_complied_clo@ to denote it to a next-stage Scala function.
...
grounded values within staging; therefore in the future stage, there will be no
monadic value.
\begin{lstlisting}
