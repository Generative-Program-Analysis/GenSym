...
generated code. \todo{put some numbers} However, we do not lose any advantages
of this high-level programming style (e.g., modularity and extensibility); the
meaning of the program also remains the same, particularly, the abstract
semantics that does the analysis when instantiated as an abstract interpreter.
...
2) Pragmatically, we show that staging abstract interpreters is an approach to
  improve performance while untouch the soundness by case studies and emprical
  evaluation. The paper is organized as follows:
...
\item We conduct three case studies on the applicability of our approach
  ...
  \item We first revisit the Abstract Compilation (AC)
    ...
    achieve the same optimization with least engineering effort.
  \item We extend the staged abstract interpreter to various control-flow
    ...
    \note{garbage collection}, showing that our approach is widely applicable.
...
\item We empirically evaluate the performance improvement by staging on
    ...
    context-insensitive, \note{context-sensitive, and store-widening} analysis.
\end{itemize}
