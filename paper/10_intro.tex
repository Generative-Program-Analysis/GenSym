...
and computes fixed-point iteratively with a cache. The solution is not simply obivious.
...
keeping the implementation as close to its conceptual model.
...
abstracts over binding-time and different semantics, which allows the staged
...
\lstinline[keywordstyle=,flexiblecolumns=false,mathescape=false,basicstyle=\tt]{eval : Expr -> Input -> Value}
is an interpreter for some language. Given a program $e_0$ of type
...
\lstinline[keywordstyle=,flexiblecolumns=false,mathescape=false,basicstyle=\tt]{Input -> Value},
