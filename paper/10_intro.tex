...
\todo{example of interpreter -> compiler, mix}
...
%artifacts of concrete semantics can be refactored to abstract interpreters.
In this paper, we study the confluence of two old ideas --- Futamura projection and
...
semantics (Figure~\ref{confluence}) in a unified framework. \todo{explain four sem}
...
to a practical problem in static analysis: can we speed up a static analyzer
without any intrusive changes and soundness compromises?
\todo{specializing analysis wrt program is not new; first staging; we argue that
staging is less intrusive compared with AC (ie, closure generation), in
fact,LMS-based staging only requires type annotations }
