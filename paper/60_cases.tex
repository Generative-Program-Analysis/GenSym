...
the \lstinline[keywordstyle=,flexiblecolumns=false,mathescape=false,basicstyle=\tt]{tick} function, which update the timestamp in the state monad. The \lstinline[keywordstyle=,flexiblecolumns=false,mathescape=false,basicstyle=\tt]{tick}
...
reusable artifact which can be used modularly.  The generated functions already
eliminate the interpretation overhead and monadic overhead, therefore can be
consider as partial summaries. Although, we still need to run the generated
function at next stage to obtain the analysis result.  At next stage, we can
provide different abstract arguments, store and caches, depending on the
context where it is called, thus the analysis is modular.
