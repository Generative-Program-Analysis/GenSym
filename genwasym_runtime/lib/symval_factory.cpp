...
MemBookKeeper<Symbolic> SymBookKeeper;
...
std::unordered_map<int, SymVal> SymbolStore;
std::unordered_map<int64_t, SymVal> FPStore;
std::unordered_map<SmallBVKey, SymVal, SmallBVKeyHash> SmallBVStore;
std::unordered_map<ExtractKey, SymVal, ExtractKeyHash> ExtractOperationStore;
std::unordered_map<BinOpKey, SymVal, BinOpKeyHash> BinaryOperationStore;
std::unordered_map<UnaryOpKey, SymVal, UnaryOpKeyHash> UnaryOperationStore;

SmallBVKey::SmallBVKey(int width, int64_t value)
    : width(width), value(value) {}

bool SmallBVKey::operator==(const SmallBVKey &other) const {
  return width == other.width && value == other.value;
}

size_t SmallBVKeyHash::operator()(const SmallBVKey &key) const {
  size_t h1 = std::hash<int>{}(key.width);
  size_t h2 = std::hash<int64_t>{}(key.value);
  return h1 ^ (h2 << 1);
}

ExtractKey::ExtractKey(const SymVal &value, int high, int low)
    : value(value), high(high), low(low) {}

bool ExtractKey::operator==(const ExtractKey &other) const {
  return value.symptr == other.value.symptr && high == other.high &&
         low == other.low;
}

size_t ExtractKeyHash::operator()(const ExtractKey &key) const {
...
BinOpKey::BinOpKey(BinOperation op, const SymVal &lhs, const SymVal &rhs)
    : op(op), lhs(lhs), rhs(rhs) {}

bool BinOpKey::operator==(const BinOpKey &other) const {
  return op == other.op && lhs.symptr == other.lhs.symptr &&
         rhs.symptr == other.rhs.symptr;
}

size_t BinOpKeyHash::operator()(const BinOpKey &key) const {
...
UnaryOpKey::UnaryOpKey(UnaryOperation op, const SymVal &value)
    : op(op), value(value) {}

bool UnaryOpKey::operator==(const UnaryOpKey &other) const {
  return op == other.op && value.symptr == other.value.symptr;
}

size_t UnaryOpKeyHash::operator()(const UnaryOpKey &key) const {
  size_t h1 = std::hash<int>{}(static_cast<int>(key.op));
