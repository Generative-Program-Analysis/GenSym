...
SymEnv_t SymEnv;
SymFrames_t SymFrames;
SymFrames_t SymGlobals;
ExploreTree_t ExploreTree;
...
SymVal SymMemory_t::loadSymFloat(int32_t base, int32_t offset) {
...
SymVal SymMemory_t::loadSymDouble(int32_t base, int32_t offset) {
...
SymVal SymMemory_t::loadSymLong32S(int32_t base, int32_t offset) {
  auto value = loadSymLong32U(base, offset);
  auto shift = SVFactory::make_concrete_bv(I64V(32), 64);
  return value.shl(shift).shr_s(shift);
}

std::monostate SymMemory_t::storeSym(int32_t base, int32_t offset, SymVal value) {
  int32_t addr = base + offset;
...
std::monostate SymMemory_t::storeSymLong(int32_t base, int32_t offset, SymVal value) {
  int32_t addr = base + offset;
...
std::monostate SymMemory_t::storeSymInt8(int32_t base, int32_t offset, SymVal value) {
  int32_t addr = base + offset;
  storeSymByte(addr, value.extract(1, 1));
  return std::monostate{};
}

std::monostate SymMemory_t::storeSymInt16(int32_t base, int32_t offset, SymVal value) {
  int32_t addr = base + offset;
  storeSymByte(addr, value.extract(1, 1));
...
std::monostate SymMemory_t::storeSymLong8(int32_t base, int32_t offset, SymVal value) {
  int32_t addr = base + offset;
  storeSymByte(addr, value.extract(1, 1));
  return std::monostate{};
}

std::monostate SymMemory_t::storeSymLong16(int32_t base, int32_t offset, SymVal value) {
  int32_t addr = base + offset;
  storeSymByte(addr, value.extract(1, 1));
  storeSymByte(addr + 1, value.extract(2, 2));
  return std::monostate{};
...
std::monostate SymMemory_t::storeSymLong32(int32_t base, int32_t offset, SymVal value) {
  int32_t addr = base + offset;
  storeSymByte(addr, value.extract(1, 1));
  storeSymByte(addr + 1, value.extract(2, 2));
...
std::monostate SymMemory_t::storeSymFloat(int32_t base, int32_t offset, SymVal value) {
  assert(value.is_concrete() && "Currently only support concrete symbolic "
                                "value for float-point values");
  return storeSym(base, offset, value);
}

std::monostate SymMemory_t::storeSymDouble(int32_t base, int32_t offset, SymVal value) {
  assert(value.is_concrete() && "Currently only support concrete symbolic "
                                "value for float-point values");
...
ImmNumMapBox::ImmNumMapBox(const NumMap &sym_env)
      : map_ptr(std::make_shared<NumMap>(
            sym_env) /* create a immutable copy of SymEnv */
        ) {}

const NumMap *ImmNumMapBox::operator->() const { return map_ptr.get(); }

const NumMap &ImmNumMapBox::operator*() const { return *map_ptr; }
...
void SymEnv_t::update(NumMap new_env) {
  map = std::move(new_env);
  imm_map_box = ImmNumMapBox(map);
}

void SymEnv_t::absorb(const NumMap &other) {
...
std::string SymEnv_t::to_string() const {
  std::string result;
  result += "(\n";
...
size_t SymEnv_t::size() const { return map.size(); }

ImmNumMapBox SymEnv_t::get_num_map() const { return imm_map_box; }

SymStack_t Snapshot_t::get_stack() const { return stack; }

SymFrames_t Snapshot_t::get_frames() const { return frames; }

SymFrames_t Snapshot_t::get_globals() const { return globals; }

SymMemory_t Snapshot_t::get_memory() const { return memory; }

Node *NodeBox::operator->() {
...
void Node::graphviz_node(std::ostream &os, const int node_id,
...
void Node::graphviz_edge(std::ostream &os, int from_id, int target_id,
...
IfElseNode::IfElseNode(SymVal cond, NodeBox *parent, int id)
...
void IfElseNode::generate_dot(std::ostream &os, int parent_dot_id,
                  const std::string &edge_label)  {
...
CallIndirectNode::CallIndirectNode(SymVal cond, NodeBox *parent, int id)
      : cond(cond), id(id),
        otherwise_branch(std::make_unique<NodeBox>(parent)) {}

std::string CallIndirectNode::to_string()  {
...
void CallIndirectNode::generate_dot(std::ostream &os, int parent_dot_id,
                  const std::string &edge_label)  {
  int current_node_dot_id = current_id;
  current_id += 1;
...
UnExploredNode::UnExploredNode() {}

std::string UnExploredNode::to_string()  { return "UnexploredNode"; }

void UnExploredNode::generate_dot(std::ostream &os, int parent_dot_id,
                  const std::string &edge_label)  {
  int current_node_dot_id = current_id++;
...
NotToExploreNode::NotToExploreNode() {}

std::string NotToExploreNode::to_string()  { return "NotToExploreNode"; }

void NotToExploreNode::generate_dot(std::ostream &os, int parent_dot_id,
                  const std::string &edge_label)  {
  int current_node_dot_id = current_id++;
...
SnapshotNode::SnapshotNode(Snapshot_t snapshot) : snapshot(snapshot) {}

std::string SnapshotNode::to_string()  { return "SnapshotNode"; }

const Snapshot_t &SnapshotNode::get_snapshot() const { return snapshot; }

Snapshot_t SnapshotNode::move_out_snapshot() { return std::move(snapshot); }

bool SnapshotNode::worth_to_reuse() const {
...
void SnapshotNode::generate_dot(std::ostream &os, int parent_dot_id,
                  const std::string &edge_label)  {
  int current_node_dot_id = current_id++;
...
std::string Finished::to_string()  { return "FinishedNode"; }

void Finished::generate_dot(std::ostream &os, int parent_dot_id,
                  const std::string &edge_label)  {
  int current_node_dot_id = current_id++;
...
std::string Failed::to_string()  { return "FailedNode"; }

void Failed::generate_dot(std::ostream &os, int parent_dot_id,
                  const std::string &edge_label)  {
  int current_node_dot_id = current_id++;
...
std::string Unreachable::to_string()  { return "UnreachableNode"; }

void Unreachable::generate_dot(std::ostream &os, int parent_dot_id,
                  const std::string &edge_label)  {
  int current_node_dot_id = current_id++;
...
void ExploreTree_t::set_cursor(NodeBox *new_cursor) {
...
std::monostate ExploreTree_t::fillFailedNode() { return cursor->fillFailedNode(); }

std::monostate ExploreTree_t::fillIfElseNode(SymVal cond, int id) {
...
std::monostate ExploreTree_t::fillCallIndirectNode(SymVal cond, int id) {
...
std::monostate ExploreTree_t::fillNotToExploredNode() {
  return cursor->fillNotToExploreNode();
}

std::vector<SymVal> ExploreTree_t::collect_current_path_conds() {
  return cursor->collect_path_conds();
}

std::monostate ExploreTree_t::moveCursor(bool branch, Control control) {
...
std::monostate ExploreTree_t::moveCursorNoControl(bool branch) {
  Profile.step(StepProfileKind::CURSOR_MOVE);
  assert(cursor != nullptr);
  auto if_else_node = dynamic_cast<IfElseNode *>(cursor->node.get());
  assert(
...
std::monostate ExploreTree_t::moveCursorIndirect(int branch_index) {
...
std::monostate ExploreTree_t::print() {
  std::cout << root->node->to_string() << std::endl;
  return std::monostate();
}

std::monostate ExploreTree_t::to_graphviz(std::ostream &os) {
  root->node->to_graphviz(os);
  return std::monostate();
}

std::monostate ExploreTree_t::dump_graphviz(std::string filepath) {
...
OverallResult ExploreTree_t::read_current_overall_result() {
...
NodeBox *ExploreTree_t::pick_unexplored() {
...
bool ExploreTree_t::all_branch_covered() const {
...
NodeBox *ExploreTree_t::get_root() const { return root.get(); }

void ExploreTree_t::register_new_node_collector(std::function<void(NodeBox *)> func) {
  new_node_collectors.push_back(func);
}

NodeBox *ExploreTree_t::pick_unexplored_of(NodeBox *node) {
...
void ExploreTree_t::register_new_node(NodeBox *node) {
  for (auto &func : new_node_collectors) {
    func(node);
  }
}

EvalRes::EvalRes(Num value, int width, ValueKind kind)
...
NodeBox::NodeBox(NodeBox *parent)
    : node(std::make_unique<UnExploredNode>()),
      /* TODO: avoid allocation of unexplored node */
      parent(parent) {}

bool NodeBox::fillIfElseNode(SymVal cond, int id) {
...
bool NodeBox::fillCallIndirectNode(SymVal cond, int id) {
...
std::monostate NodeBox::fillSnapshotNode(Snapshot_t snapshot) {
...
std::monostate NodeBox::fillNotToExploreNode() {
  if (this->isUnexplored()) {
...
std::monostate NodeBox::fillFinishedNode() {
  if (this->isUnexplored()) {
...
std::monostate NodeBox::fillFailedNode() {
  if (this->isUnexplored()) {
...
std::monostate NodeBox::fillUnreachableNode() {
  if (this->isUnexplored()) {
...
bool NodeBox::isSnapshotNode() const {
  assert(node != nullptr);
  return dynamic_cast<SnapshotNode *>(node.get()) != nullptr;
}

bool NodeBox::isUnexplored() const {
  assert(node != nullptr);
...
std::vector<SymVal> NodeBox::collect_path_conds() {
...
void Snapshot_t::restore_states_to_global() const {
