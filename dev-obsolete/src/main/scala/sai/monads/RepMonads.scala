...
trait RepMonads extends RepLattices { self: SAIDsl =>
  ...
  object SetStateReaderStateM {
    ...
    implicit def SetStateReaderStateMonad[R: Manifest, S1: Manifest, S2: Manifest] = new Monad[SetStateReaderStateM[R,S1,S2,?]] {
        ...
        def pure[A: Manifest](a: Rep[A]): SSRS[R,S1,S2,A] = SetStateReaderStateM(s1 => r => s2 => ((Set[A](a),s1),s2))
        def filter[A: Manifest](ma: SSRS[R,S1,S2,A])(f: Rep[A] => Rep[Boolean]): SSRS[R,S1,S2,A] = ma.filter(f)

        def put1(s1: => Rep[S1]): SSRS[R,S1,S2,Unit] = SetStateReaderStateM(_ => r => s2 => ((Set[Unit](()), s1), s2))
        def put2(s2: => Rep[S2]): SSRS[R,S1,S2,Unit] = SetStateReaderStateM(s1 => r => _ => ((Set[Unit](()), s1), s2))
        ...
        def mod1(f: Rep[S1] => Rep[S1]): SSRS[R,S1,S2,Unit] = SetStateReaderStateM(s1 => r => s2 => ((Set[Unit](()), f(s1)), s2))
        def mod2(f: Rep[S2] => Rep[S2]): SSRS[R,S1,S2,Unit] = SetStateReaderStateM(s1 => r => s2 => ((Set[Unit](()), s1), f(s2)))
