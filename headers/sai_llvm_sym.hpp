...
/*
struct Loc {
  friend std::ostream& operator<<(std::ostream&os, const Loc& v) {
...
struct FrameLoc : Loc {
  String s;
  FrameLoc(String s) : s(s) {}
  virtual std::ostream& toString(std::ostream& os) const override {
    return os << "FrameLoc(" << s << ")";
  }
};
*/

using Loc = String;
using FrameLoc = String;
using AllocaLoc = String;

inline String make_FrameLoc(String s) {
  return s;
}
inline String make_AllocaLoc(String s) {
  return s;
}

/*
inline Ptr<FrameLoc> make_FrameLoc(String s) {
  return std::make_shared<String>(s);
  //return std::make_shared<FrameLoc>(s)
};

struct AllocaLoc : Loc {
  String s;
  AllocaLoc(String s) : s(s) {}
  virtual std::ostream& toString(std::ostream& os) const override {
    return os << "AllocaLoc(" << s << ")";
  }
};

inline Ptr<AllocaLoc> make_AllocaLoc(String s) {
  return std::make_shared<String>(s);
  //return std::make_shared<AllocaLoc>(s)
};
*/
...
inline Ptr<LocV> make_LocV(Loc l) {
  return std::make_shared<LocV>(l);
}

//inline Ptr<Loc> proj_LocV(Ptr<Value>& v) {
inline Loc proj_LocV(Ptr<Value>& v) {
...
const static Ptr<Value> bot = std::make_shared<BotV>();
...
const static auto& LocV_ty = typeid(struct LocV);
const static auto& BotV_ty = typeid(struct BotV);
...
inline bool operator==(const Ptr<LocV>& lhs, const Ptr<LocV>& rhs) {
  return lhs->l.compare(rhs->l) == 0;
}

inline bool operator==(const Ptr<BotV>& lhs, const Ptr<BotV>& rhs) {
  return true;
}
...
inline bool operator==(const Ptr<Value>& lhs, const Ptr<Value>& rhs){
  ...
  else if (lhs_ty == LocV_ty && rhs_ty == LocV_ty) {
    return std::dynamic_pointer_cast<LocV>(lhs) ==
      std::dynamic_pointer_cast<LocV>(rhs);
  }
  /*
  ...
  */
  else if (lhs_ty == BotV_ty && rhs_ty == BotV_ty) {
    return true;
  }
...
namespace std {
  ...
  */
  ...
  template<>
  struct hash<BotV> {
    std::size_t operator()(const BotV& v) const {
      return 0;
    }
  };
  ...
  template<>
  struct hash<LocV> {
    std::size_t operator()(const LocV& v) const {
      return std::hash<Loc>{}(v.l);
    }
  };
  ...
  struct hash<Ptr<Value>> {
    std::size_t operator()(const Ptr<Value>& v) const {
      ...
      else if (v_ty == BotV_ty) {
        return 0;
      }
      else if (v_ty == LocV_ty) {
        return std::hash<LocV>{}(*std::dynamic_pointer_cast<LocV>(v));
      }
      /*
