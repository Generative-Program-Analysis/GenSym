...
def compile_all(cpp_files=None, flags=None):
    ...
    for cpp in cpp_files:
        ...
        cmd = [CPP, str(cpp), "-o", str(out)] + flags
        ...
        try:
            ...
            print(f"Error: {CPP} not found. Install a C++ compiler.")
            sys.exit(1)

        snapshot_out = cpp.with_suffix(".snapshot.exe")
        snapshot_cmd = [CPP, str(cpp), "-o", str(snapshot_out)] + SNAPSHOT_FLAGS
        ...
        snapshot_cost_model_cmd = [CPP, str(cpp), "-o", str(snapshot_cost_model_out)] + SNAPSHOT_COST_MODEL_FLAGS
...
if __name__ == "__main__":
    cpps = [HERE / "parse_expr3.wat.cpp", HERE / "parse_expr4.wat.cpp"]
    compile_all()
