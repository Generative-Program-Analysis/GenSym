...
int main() {
  for (int i=0; i<10;i++) {
    gs_assert_eager(b[i] == ((i+1)%10));
  }
  for (int i=0; i<4;i++) {
    gs_assert_eager(c[i] == d[i]);
  }
  gs_assert_eager(c[0] == 92);
  gs_assert_eager(c[1] == 54);
  gs_assert_eager(c[2] == 49);
  gs_assert_eager(c[3] == 0);
  for (int i=0; i<9;i++) {
    gs_assert_eager(e[i] == f[i]);
  }
}