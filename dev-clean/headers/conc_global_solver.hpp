...
inline bool isPairV(PtrVal v) {
  if (std::dynamic_pointer_cast<PairV>(v)) return true; else return false;
}
...
inline void conc_main_single(Input args, Input sargs) {
  immer::flex_vector<SExpr> this_pc = conc_exec(args, sargs);
  // int res = 0;
  // for (int i = 0; i < this_pc.size(); i++) {
  //   auto pc_prime = this_pc.take(i).push_back(to_SMTBoolNeg(this_pc[i]));
  //   res += check_pc_to_file(pc_prime);
  // }
  // std::cout << "generated " << res << " new inputs" << std::endl;
}
