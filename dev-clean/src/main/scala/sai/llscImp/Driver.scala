...
object RunLLSC {
  @virtualize
  def specializeCPS(m: Module, name: String, fname: String, nSym: Int): CPSLLSCDriver[Int, Unit] =
  ...
  def runCPSLLSC(m: Module, name: String, fname: String, nSym: Int = 0) {
    val (_, t) = sai.utils.Utils.time {
      val code = specializeCPS(m, name, fname, nSym)
      code.genAll
    }
