...
trait ConcolicOptEngine extends SAIOps with StagedNondet with SymExeDefs {
  ...
  // Note: the desired behavior for symbolic eval is to return concrete value if symVal is not presented
  ...
  def execTerm(inst: Terminator, incomingBlock: String)(implicit funName: String, isSym: Boolean): Comp[E, Rep[Value]] = {
    inst match {
      case Unreachable => ret(PairV(NullV(), NullV()))
      case RetTerm(ty, Some(value)) =>
        for {
          sv <- eval(value, ty)(funName, true)
          v <- eval(value, ty)(funName, false)
        } yield PairV(v, sv)
      case RetTerm(ty, None) => ret(PairV(NullV(), NullV()))
      case BrTerm(lab) =>
        for {
  ...
  // def doIfSym(v: Rep[Value], s: Rep[SS], c: Rep[Value] => Comp[E, Rep[Unit]]): Comp[E, Rep[Unit]] = reflect {
  //   if (!v.isConc) reify(s)(c(v))
  //   else (s, ())
  // }
